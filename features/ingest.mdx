...
Use files when you know the exact documents to index. File entries can be local paths, HTTP/HTTPS file links, upload bytes in the SDK, or explicit dictionaries with `kind`, `path`, `url`, `folder`, `title`, `description`, and `metadata`.
...
result = rag.ingest(
    docs=[
        ...
        {
            "kind": "url",
            "url": "https://example.com/files/product-guide.pdf",
            "title": "Product guide",
            "description": "Hosted PDF guide",
        },
    ],
    ingestion={"docs": {"frontmatter": True}},
...
<Tip>For browser/app uploads that belong in the same indexing run as paths, folders, URLs, websites, APIs, or Markdown, use [`POST /v1/pipelines/ingest/mixed`](/usage/server/ingest#mixed-document-uploads).</Tip>

## Mixed Document Items

The canonical document item schema uses explicit `kind` values. SDK calls can mix upload bytes, paths, remote file URLs, and folders in one `docs` list. REST JSON ingest supports path, URL, and folder items; REST multipart mixed ingest adds upload items.

```python SDK
from pathlib import Path

result = rag.ingest(docs=[
    {"kind": "upload", "content": Path("files/refund.pdf").read_bytes(), "filename": "refund.pdf"},
...
result = rag.ingest(
    ...
    ingestion={"urls": {"verbose": False, "frontmatter": True}},
    chunking={"chunk_size": 2000, "chunk_overlap": 200, "min_chunk_length": 100},
    embedding={"batch_size": 64},
    storage={"batch_size": 64, "ensure_collection": True},
)
...
result = rag.ingest(
    ...
    ingestion={"api": {"frontmatter": True}},
    chunking={"chunk_size": 2000, "chunk_overlap": 200, "min_chunk_length": 100},
    embedding={"batch_size": 64},
    storage={"batch_size": 64, "ensure_collection": True},
)

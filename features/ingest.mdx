...
<Tip>For browser/app uploads, use the multipart extraction endpoint first: [REST extraction](/usage/server/ingestion#upload-documents-post-v1ingestdocsupload).</Tip>
...
<Tip>For a copy-paste flow, start with the [Quickstart](/getting-started/quickstart). For interface-specific details, see [SDK pipeline](/usage/sdk/pipeline), [CLI pipeline](/usage/cli/pipeline), or [REST pipeline](/usage/server/pipeline).</Tip>

...
# Disable YAML frontmatter on saved markdown output
ragrails scrape https://example.com/docs \
  --no-frontmatter \
  --output-dir files/output/web/
```
...
with open("files/guide.pdf", "rb") as f:
    data = f.read()
bytes_result = rag.parse(files=[{"kind": "upload", "content": data, "filename": "guide.pdf", "title": "Guide"}])

# Mixed inputs in one call
mixed = rag.parse(files=[
    {"kind": "path", "path": "files/local.pdf"},
    {"kind": "url", "url": "https://example.com/files/remote.docx"},
    {"kind": "upload", "content": data, "filename": "upload.pdf", "source": "s3://bucket/upload.pdf"},
    {"kind": "folder", "folder": "files/policies/"},
])

# Frontmatter is enabled by default for markdown output; disable it only when needed
plain = rag.parse(files="files/guide.pdf", frontmatter=False)
...
# Disable frontmatter on saved markdown output
ragrails parse --files files/guide.pdf --no-frontmatter --output-dir files/output/docs/
```
...
<Note>Use JSON `/v1/ingest/docs` for files reachable by the server process. Use multipart `/v1/ingest/docs/upload` when a client is only uploading file bytes for parsing. Use `/v1/pipelines/ingest/mixed` when uploads must be indexed together with paths, folders, URLs, websites, APIs, or Markdown.</Note>

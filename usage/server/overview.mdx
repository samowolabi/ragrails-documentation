...
| Dependency preflight | `GET /v1/health/deps` |

Run the dependency preflight before your first pipeline request. It reports missing optional packages, provider keys, and common local-vs-Docker vector database URL mistakes.

```bash
curl http://127.0.0.1:8000/v1/health/deps
```
...
| --- | --- | --- |
| [Ingest](/usage/server/ingest) | `POST /v1/pipelines/ingest`, `POST /v1/pipelines/ingest/mixed` | Extract, chunk, embed, and store content in one request. Use the mixed endpoint when browser uploads and JSON document sources belong in the same pipeline. |
...
Most request and response bodies are JSON. Document uploads use multipart form data. Use `/v1/pipelines/ingest/mixed` when a single ingest request must combine uploaded files with server paths, folders, remote document URLs, websites, APIs, or Markdown. Use the OpenAPI schema when generating clients.

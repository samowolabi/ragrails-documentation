...
| POST | `/v1/ingest/url/stream` | Stream URL scrape events |
...
| POST | `/v1/chat/stream` | Stream RAG chat events |

Most request and response bodies are JSON. Streaming endpoints return `text/event-stream`. See the Swagger UI for full request schemas.

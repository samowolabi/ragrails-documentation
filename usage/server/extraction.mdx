...
<Note>Multipart uploads are limited by server config. By default, `/v1/ingest/docs/upload` and `/v1/pipelines/ingest/mixed` accept up to 20 files and 50 MB per file. Set `RAGRAILS_MAX_UPLOAD_FILES` and `RAGRAILS_MAX_UPLOAD_BYTES` in the server environment to change those limits. Requests over either limit return HTTP `413`.</Note>

<Tip>Use [`/v1/pipelines/ingest/mixed`](/usage/server/ingest#mixed-document-uploads) when uploaded files must be indexed in the same pipeline request as server paths, folders, remote document URLs, websites, APIs, or Markdown.</Tip>
...
| `/v1/pipelines/ingest/mixed` | multipart `manifest`, `files`, optional `file_refs` |

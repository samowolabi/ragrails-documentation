...
`POST /v1/pipelines/ingest` extracts, chunks, embeds, and stores JSON-addressable content. Use `POST /v1/pipelines/ingest/mixed` when browser uploads must be indexed in the same request as paths, folders, URLs, APIs, or Markdown.

...
REST requests can be self-contained, or they can omit provider fields and use server environment defaults. Put provider and vector-store settings in the request body when one request needs to override the server defaults.
...
Server-level limits live in the process environment:

```bash
export EMBEDDING_PROVIDER=voyage
export EMBEDDING_MODEL=voyage-3
export VECTOR_DB_PROVIDER=qdrant
export VECTOR_DB_URL=http://qdrant:6333
export VECTOR_DB_COLLECTION=rag_chunks
export RAGRAILS_MAX_UPLOAD_FILES=20
export RAGRAILS_MAX_UPLOAD_BYTES=52428800
```

`EMBEDDING_PROVIDER`, `EMBEDDING_MODEL`, `VECTOR_DB_PROVIDER`, `VECTOR_DB_URL`, and `VECTOR_DB_COLLECTION` are REST server defaults used when request bodies omit those fields. `RAGRAILS_MAX_UPLOAD_FILES` controls how many files `/v1/ingest/docs/upload` and `/v1/pipelines/ingest/mixed` accept in one multipart request. `RAGRAILS_MAX_UPLOAD_BYTES` controls the maximum size of each uploaded file in bytes. Uploads above either limit return HTTP `413`.

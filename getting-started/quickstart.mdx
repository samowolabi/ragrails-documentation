...
<CodeGroup>
```python SDK sources
# Documents: parse local files, folders, remote file URLs, or upload bytes.
rag.ingest(docs=["files/refund-policy.pdf"])
rag.ingest(docs={"folder": "files/policies"})
rag.ingest(docs=[
    {"kind": "path", "path": "files/refund-policy.pdf"},
...
# JSON ingest can use server-readable files, folders, and remote file URLs.
# For browser uploads in the same pipeline, use /v1/pipelines/ingest/mixed.
curl -X POST http://127.0.0.1:8000/v1/pipelines/ingest \
  -H "Content-Type: application/json" \
  -d '{

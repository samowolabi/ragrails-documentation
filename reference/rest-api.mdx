...
| GET | `/v1/health/deps` | Check REST dependencies, provider keys, and common setup mistakes. |
...
| POST | `/v1/pipelines/ingest/mixed` | Run ingest with a multipart manifest and uploaded files. |
...
`POST /v1/ingest/docs/upload` and `POST /v1/pipelines/ingest/mixed` are limited by `RAGRAILS_MAX_UPLOAD_FILES` and `RAGRAILS_MAX_UPLOAD_BYTES` in the server environment. Requests above either limit return HTTP `413`.

Use `/v1/pipelines/ingest/mixed` when browser uploads must be indexed in the same pipeline request as path, folder, URL, website, API, or Markdown sources.
